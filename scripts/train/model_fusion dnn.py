...
class Fused_DNN(nn.Module):
    def __init__(self, models, num_classes, dropout=0.5, hidden=[200, 100, 50], in_chans=3, normalize=True):
        super().__init__()
        ...
        self.dense = nn.Sequential(
			...
			nn.ReLU(inplace=True),
   			nn.Dropout(dropout),
            nn.LazyLinear(hidden[1]),
			nn.ReLU(inplace=True),
   			nn.Dropout(dropout),
            nn.LazyLinear(hidden[2]),
			nn.ReLU(inplace=True),
   			nn.Dropout(dropout),
    ...
    def forward(self, x):
        ...
        for model in self.models:
            with torch.no_grad():
                features = model.forward_head(x) # (batch_size, feature_count)
...
def parse_args():
    ...
    parser.add_argument('--drop', type=float, default=0.5, metavar='d',
                   help='portion of neurons to drop (default: 0.5)')
